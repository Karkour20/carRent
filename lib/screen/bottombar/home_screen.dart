...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<List<myBanner>> fetchProductsFromFirebase() async {
    // Access the Firestore instance
    FirebaseFirestore firestore = FirebaseFirestore.instance;

    // Get the collection reference
    CollectionReference productsRef = firestore.collection('banners');

    QuerySnapshot querySnapshot = await productsRef.get();

    // Extract data from the query snapshot
    List<myBanner> banner= querySnapshot.docs.map((doc) {
      return myBanner(
          id: doc["id"],
          img: doc["img"]
...
Future<List<RecommendCar>> recommendCar() async {
  // Access the Firestore instance
  FirebaseFirestore firestore = FirebaseFirestore.instance;
  ...
  // Extract data from the query snapshot
  List<RecommendCar> recommendCar = querySnapshot.docs.map((doc) {
    return RecommendCar(
  ...
  Future<List<Carlist>> fechcarlist() async {
    // Access the Firestore instance
    FirebaseFirestore firestore = FirebaseFirestore.instance;

    // Get the collection reference
    CollectionReference productsRef = firestore.collection('carlists');

    QuerySnapshot querySnapshot = await productsRef.get();

    // Extract data from the query snapshot
    List<Carlist> carlist= querySnapshot.docs.map((doc) {
      return Carlist(
          id: doc["id"],
          img: doc["img"],
          title:doc["title"], 
  ...
  Future<List<Carbrand>> carbrand2() async {
    // Access the Firestore instance
    FirebaseFirestore firestore = FirebaseFirestore.instance;

    // Get the collection reference
    CollectionReference productsRef = firestore.collection('Carbrand');

    QuerySnapshot querySnapshot = await productsRef.get();

    // Extract data from the query snapshot
    List<Carbrand> carbrand1= querySnapshot.docs.map((doc) {
      return Carbrand(
          id: doc["id"],
          img: doc["img"],
          title:doc["title"],
  ...
  homeBanner(uid) async {

    List<myBanner> mybanners =await fetchProductsFromFirebase();
    List<Carlist> carlists =await fechcarlist();
    List<FeatureCar> featureCar =await featuredcars();
    List<Carbrand> carbrand =await carbrand2();
      ...
      banner = HomeBanner(
        ...
        recommendCars: [
          RecommendCar(
            ...
            engineHp: "280",
            fuelType: "Petrol",
            carDistance: "12,000 miles",
            carTitle: 'tesla',
            carImg: ["https://picsum.photos/250?image=9","https://picsum.photos/250?image=9","https://picsum.photos/250?image=9"],
            carRating: '5',
            carGear: 'otomatic',
            carTypeTitle: 'bmw2',
          ),
                                ...
                                itemBuilder: (context, index, realIndex) {
                                  return Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      child: CachedNetworkImage(imageUrl: "${Config.imgUrl}${banner.banner[index].img}",fit:BoxFit.fitWidth,)
                                  );
                                },
                              ...
                              SizedBox(
                                ...
                                child:  banner.featureCar.isEmpty ? ematyCar(title: 'Featured Cars'.tr,colors: notifire.getwhiteblackcolor) :  ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    return InkWell(
                                      onTap: () {
                                        Navigator.of(context).push(MaterialPageRoute(builder: (context) => CarDetailsScreen(id: banner.featureCar[index].id, currency:  banner.currency, carInfo: CarInfo(carinfo:banner.featureCar[index] , galleryImages:banner.featureCar[index].carImg, responseCode: "responseCode", result: "result", responseMsg: "responseMsg"),)));
                                      },
                                      child: Container(
                                        height: 330,
                                        width: 266,
                                        ...
                                        child: ClipRRect(
                                          ...
                                          child: Stack(
                                            fit: StackFit.expand,
                                            children: [
                                              Image.network("${Config.imgUrl}${banner.featureCar[index].carImg.first}", fit: BoxFit.cover),
                                              Container(
                                                decoration: BoxDecoration(
