...
import 'package:carlink/payments/payTapsClass.dart';
...
class ReviewSummery extends StatefulWidget {
  ...
  final String sTime;
  final String eTime;
  final FeatureCar featureCar;
  final DateTime startDateTime;
  final DateTime endDateTime;
  const ReviewSummery({super.key, required this.Picktime, required this.returnTime, required this.toggle, required this.startDate, required this.endDate, required this.hours, required this.days, required this.sTime, required this.eTime, required this.featureCar, required this.startDateTime, required this.endDateTime});
...
class _ReviewSummeryState extends State<ReviewSummery> {
  ...
  Future bNow(carId, uId, carPrice, pType, pDate, pTime, rDate, rTime, couponId, couponAmt, wallAnt, totalDay, subtotal, tax, tAmt, tfinal, pMethod, paymentId, typeId, brandId, bookType, cityId) async {
    ...
    DocumentReference docRef = FirebaseFirestore.instance.collection('Books').doc();
    ...
    Map<String, dynamic> body2 = {
      ...
      "book_type": bookType,
      "city_id": cityId,
      // Additional variables from BookDetail class
      "book_id": documentId,
      "car_title": widget.featureCar.carTitle,
      "car_number": widget.featureCar.carNumber,
      "car_img": widget.featureCar.carImg[0],
      "pick_lat": widget.featureCar.pickLat,
      "pick_lng": widget.featureCar.pickLng,
      "pick_address": widget.featureCar.pickAddress,
      "city_title": widget.featureCar.cityId,
      "car_rating": widget.featureCar.carRating,
//todo owner data
      "owner_name": "karkour",
   ...
   // try{
      ...
      var carRef = FirebaseFirestore.instance.collection('featuredcars').doc(carId);
         ...
         var book = await docRef.set(body2);
      widget.featureCar.bookeddate!.removeWhere((element) =>  element.returnDate.isBefore(DateTime.now()));

      widget.featureCar.bookeddate!.add(Bookeddate(pickupDate:widget.startDateTime , returnDate: widget.endDateTime));
         // delete former bookeddate
         carRef.update({
           "bookeddate":List<dynamic>.from(widget.featureCar.bookeddate!.map((x) => x.toFirebase()))
        ...
        setState(() {
          isLoads=false;
            ...
            Fluttertoast.showToast(msg: "booked successfully".tr,timeInSecForIosWeb: 4);
  ...
  PayTapsClass payTapsClass = PayTapsClass();
  ...
  void handlePayTapsSuccess( response) {
    bNow(car['id'], name.uid, widget.toggle == true ? car['car_rent_price_driver'] : car['car_rent_price'], car['price_type'] == '1' ? 'hr' : 'days', widget.startDate, widget.sTime, widget.endDate, widget.eTime, coupon == 1 ? cList?.couponlist[inDex].id : '0', cAmt ?? '0', walletValue, car['price_type'] == '1' ? widget.hours : widget.days, total.toStringAsFixed(2), tax['tax'], totalTax, totalPayment.toStringAsFixed(2), gPayment?.paymentdata[0].id, "response.paymentId", car['type_id'], car['brand_id'], widget.toggle == true ? 'With' : 'Without', car['city_id']);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        setState(() {
          ...
          Paymentdatum paytabsdatum= Paymentdatum(
              id: "id", title: "PayTabs", img: "images/payment/1672813747.png", attributes: "attributes", status: "1", subtitle: "PayTabs is local payment gateway", pShow: "1");
          gPayment?.paymentdata.insert(0, paytabsdatum);
          Loading = false;
        });
      } else {}
    } catch (e) {}
  ...
  cData() async {
    ...
    tax ={
      'tax':10
        ...
        //jsonDecode(prefs.getString('bannerData')!);
     total =
     ...
     ;
     //print("this is total ${total * double.parse(tax['tax']) / 100}");
    totalTax = total * tax['tax'] / 100;
    fTotal = total + totalTax;
    totalPayment = fTotal;
  }
                      ...
                      amount:totalPayment ,
                          ...
                          if (event["status"] == "success") {
                            ...
                            if (transactionDetails["isSuccess"]) {
                              ...
                              Fluttertoast.showToast(msg: "Transaction Successful", timeInSecForIosWeb: 4);

                              handlePayTapsSuccess(transactionDetails);
                                          ...
                                          child: ListTile(
                                            leading: Transform.translate(
                                              ...
                                              child: Container(
                                                ...
                                                child: Image.network(
                                                    "${"https://carlink.cscodetech.cloud/"}${gPayment?.paymentdata[index].img}"),
                                              ),
                                            ),
                                            title: Padding(
                                              padding:
