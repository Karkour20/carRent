...
class _MyPurchasesScreenState extends State<MyPurchasesScreen> {
  ...
  //  try{
var data=await FirebaseFirestore.instance.collection("Books").where("uid",isEqualTo: FirebaseAuth.instance.currentUser!.uid)
    //.where("status",isEqualTo: status)
    .get();
      //if(response.statusCode == 200){
        setState(() {
          bookHistory = bookHistoryModalFromJson({
  ...
  Future bHistory1(uid, status) async {
    ...
    try{
      ...
      print(data.docs);
      setState(() {
        bookHistory1 = bookHistoryModalFromJson({
          "book_history": data.docs.map((e) => e.data()).toList(),
          "ResponseCode": "responseCode",
          "Result": "result",
          "ResponseMsg": "responseMsg",
