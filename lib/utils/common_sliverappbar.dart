...
class CollapsingAppBarPage extends StatefulWidget {
  ...
  const CollapsingAppBarPage({super.key, required this.bodyCreator, required this.carTitle, required this.Id, required this.favorite, this.carInfo});
...
class _CollapsingAppBarPageState extends State<CollapsingAppBarPage> with SingleTickerProviderStateMixin {
  ...
  Future carInfo(uid) async {
    ...
    try{
      ...
      if(response.statusCode == 200){
        setState(() {
         // widget.carInfo! = carInfoFromJson(response.body);
          loading = false;
        });
        var data = jsonDecode(response.body.toString());
        return data;
            ...
            children: [
              Container(
                ...
                child: PageView.builder(
                  ...
                  itemCount: widget.carInfo!.carinfo.carImg.length,
                  itemBuilder: (_, index) {
                    return Stack(
                      clipBehavior: Clip.none,
                      children: [
                        SizedBox(
                          height: 500,
                            width: Get.width,
                            child: Image.network('${Config.imgUrl}${widget.carInfo!.carinfo.carImg[index]}', fit: BoxFit.cover)),
                      ],
                    );
                  },
                ),
              ...
              widget.carInfo!.carinfo.carImg.length == 1 ? const SizedBox() :Positioned(
                left: 0,
                right: 0,
                bottom: 20,
                child: Center(
                  child: SmoothPageIndicator(
                    controller: controller,
                    count: widget.carInfo!.carinfo.carImg.length,
